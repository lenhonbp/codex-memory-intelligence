...
export async function loadTrackedMemory(root) {
  ...
  const entries = [];
  for (const file of MEMORY_FILES) try { entries.push(...parseEntries(await fs.readFile(path.join(directory, file), 'utf8'), file)); } catch {}
  return entries;
}
export async function checkStaleMemory(root) {
  ...
  for (const entry of entries) {
    ...
    if (!meta) { results.push({ id: null, file: entry.file, heading: entry.heading, text: entry.text, status: 'untracked', reasons: ['Entry predates metadata tracking. Refresh it to establish a baseline.'] }); continue; }
    const reasons = [];
    let status = 'fresh';
    ...
    results.push({ id: meta.id || null, type: meta.type || entry.file.replace('.md', ''), file: entry.file, heading: entry.heading, text: entry.text, sources, ageDays, status, reasons, reviewedBy: meta.reviewedBy || null, reviewReason: meta.reviewReason || null });
  }
...
export async function refreshMemory(root, selector = 'all', options = {}) {
  ...
  const index = await readJson(path.join(directory, 'project-index.json'));
  let updated = 0;
  const reviewedBy = String(options.reviewedBy || 'human').trim().slice(0, 100) || 'human';
  const reviewReason = String(options.reason || 'Reviewed against the current project.').trim().slice(0, 500) || 'Reviewed against the current project.';
  ...
  return { selector, updated, reviewedBy, reviewReason };
}
