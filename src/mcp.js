...
function writable() { if (!writeEnabled) throw new Error('MCP durable project writes are disabled. Generate config with cmi mcp-config --write to enable scans, reviewed memory, and durable change/session records.'); }
...
async function callTool(name, args = {}) {
  ...
  if (name === 'list_project_workspaces') {
    const result = await status(root);
    if (!result.workspaces) throw new Error('Project workspace index is missing. Run scan_project_intelligence in write-enabled MCP mode first.');
  ...
  if (name === 'get_project_graph') {
    ...
    if (!result) throw new Error('Project graph is missing. Run scan_project_intelligence in write-enabled MCP mode first.');
...
const readTools = [
  ...
  { name: 'prepare_change_brief', title: 'Prepare change brief', description: 'Build a professional pre-change brief combining Git baseline, ranked context, inferred boundaries, impact, memory gaps, risks, verification, confidence, provenance, and historical change evidence.', inputSchema: { type: 'object', required: ['query'], properties: { query: { type: 'string' }, limit: { type: 'integer', minimum: 1, maximum: 30 }, depth: { type: 'integer', minimum: 1, maximum: 8 }, workspace: { type: 'string' } } }, annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false } },
...
const writeTools = [
  ...
  { name: 'complete_change_record', title: 'Record Change completion or partial progress', description: 'Capture AFTER evidence, outcome, verification claims/provenance, unexpected impact, prediction gaps, and review-only learning candidates. A partial outcome preserves the Change as active; terminal outcomes make it immutable.', inputSchema: { type: 'object', required: ['id'], properties: { id: { type: 'string' }, outcome: { type: 'string', enum: ['succeeded', 'failed', 'partial', 'abandoned', 'unknown'] }, files: { type: 'array', items: { type: 'string' }, maxItems: 160 }, verifications: { type: 'array', maxItems: 20, items: verificationSchema }, unexpectedImpact: { type: 'array', maxItems: 20, items: { type: 'string', maxLength: 500 } }, notes: { type: 'array', maxItems: 20, items: { type: 'string', maxLength: 500 } } } }, annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false } },
...
async function readResource(uri) {
  ...
  if (uri === 'cmi://project/graph-summary') {
    ...
    if (!graph) throw new Error('Project graph is missing. Run scan_project_intelligence in write-enabled MCP mode first.');
...
function promptResult(name, args = {}) {
  ...
  if (name === 'run_change_intelligence_loop') {
    ...
    return { description: `Track evidence for ${target}.`, messages: [{ role: 'user', content: { type: 'text', text: `Run CMI's change intelligence loop for ${target}.${workspace} BEFORE: inspect get_change_insights, then call start_change_record and review its predicted scope, historical evidence, risks, and verification plan. DURING: implement the smallest justified change and call observe_change_record after meaningful edits; provide explicit project-relative files only when Git cannot attribute them. AFTER: if MCP write mode is enabled, rescan project intelligence and run the project's real verification commands yourself. Complete the Change with outcome and verification statuses/provenance only when the requested work is finished. If work is intentionally partial, paused, or awaiting review, preserve the evidence and leave the Change active, then close only the work session. Review prediction gaps and learning candidates, but never convert them into project memory automatically. Treat changed paths as observed evidence, not proof of complete runtime impact or causality.` } }] };
  }
  if (name === 'review_stale_memory') {
    return { description: 'Review project knowledge health and lifecycle before mutating it.', messages: [{ role: 'user', content: { type: 'text', text: 'Run the memory health check. For each active stale or review item, inspect its linked sources and current architecture. Refresh source fingerprints only when requested, and do not treat that refresh as semantic review. If knowledge is no longer trustworthy or applicable, use the reviewed lifecycle tool to deprecate, reject, or supersede it instead of silently deleting it. Supersession must point to an active replacement entry.' } }] };
  }
