...
function promptResult(name, args = {}) {
  ...
  if (name === 'run_change_intelligence_loop') {
    ...
    return { description: `Track evidence for ${target}.`, messages: [{ role: 'user', content: { type: 'text', text: `Run CMI's change intelligence loop for ${target}.${workspace} BEFORE: inspect get_change_insights, then call start_change_record and review its predicted scope, historical evidence, risks, and verification plan. DURING: implement the smallest justified change and call observe_change_record after meaningful edits; provide explicit project-relative files only when Git cannot attribute them. AFTER: if MCP write mode is enabled, rescan project intelligence; run the project's real verification commands yourself, then call complete_change_record with outcome and verification statuses/provenance. Review prediction gaps and learning candidates, but never convert them into project memory automatically. Treat changed paths as observed evidence, not proof of complete runtime impact or causality.` } }] };
  }
