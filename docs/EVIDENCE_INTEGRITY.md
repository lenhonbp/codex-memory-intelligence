...
Portable evidence is not an authenticated backup: SHA-256 detects corruption but does not authenticate the bundle or prove source authorship. Source identity is content-based and repository/revision-aware when Git evidence is available; absolute paths are diagnostic only. Valid UTF-8 source text is normalized to LF for identity comparison so clean Git checkouts with different line-ending conventions remain compatible; binary source content remains byte-exact.
