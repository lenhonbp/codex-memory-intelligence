...
| CLI/MCP contract parity | Ready for Phase 3 review | Shared runtime contracts and existing parity tests, matrix below records the public surface, and the public-surface lifecycle is now governed by [`DEPRECATION_POLICY.md`](DEPRECATION_POLICY.md) (announced deprecation, two-release minimum transition, fail-closed removal, security exception) | Deprecation policy exists; v1 stability promise remains a separate future decision |
...
| --- | --- | --- |
| Reviewed memory and metadata | Marker-free legacy text remains untracked; valid legacy metadata remains readable; a present invalid/future marker blocks retrieval and every normal memory mutation without rewrite | The audited historical floor is read-compatible without migration; future schema changes still require a separately justified explicit migration or supported-floor decision |
| Project graph, index, generated architecture, and snapshots | Old graph schema/parser state is obsolete/rebuildable; future graph/parser/index state, including mixed pairs, is unsupported and blocks normal scan | Rebuild is the fallback only for old generated artifacts; future formats require compatible/newer CMI or explicit preserve/remove recovery and must never receive an ordinary scan recommendation |
| Configuration | `.codex-memory/config.json` stores user/project scan policy and resolver inputs used by current evidence; future versions block status/doctor configuration health and configuration-dependent graph trust before any write | Preserve settings through an explicit validated migration only if a future observed incompatibility requires it; do not discard or silently reset user configuration |
...
| Unsupported/future memory | A future memory marker was blocked with an explicit code; search/context/prepare plus remember/all-targeted refresh/lifecycle mutations refused it. | Accepted; durable Markdown bytes unchanged |
| Unsupported/future config | A healthy project with only config version set to `999` produced blocked status/doctor configuration diagnostics and non-current graph health. | Accepted; config bytes unchanged, no defaults or downgrade path |
| Unsupported/future generated state | Paired and mixed future/current graph/index states blocked evidence and normal scan without an ordinary scan recommendation. | Accepted; generated bytes unchanged; compatible/newer CMI or explicit preserve/remove recovery only |
...
Study 001 (Issue #28) remains an incomplete historical study and is closed as `not planned` for archival disposition, not as a completed paired study. The preserved issue/preregistration evidence shows the plain condition complete and the CMI condition missing/unrecoverable, with no original CMI condition artifact of sufficient provenance. No product claim is made, no new condition is run, and Study 002 or later evaluator commentary is not used to manufacture the missing condition.
